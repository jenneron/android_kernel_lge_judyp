...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	if (unified_bootmode_fabproc()
		...
		if (smblib_read(chg, APSD_RESULT_STATUS_REG, &reg) >= 0
			...
			smblib_dbg(chg, PR_MISC, "Skip to set ICL OVERRIDE for factory cable\n");
		smblib_set_charge_param(chg, &chg->param.usb_icl, 1500000);
		override = true;
		goto suspend_config;
	}
#endif
...
{	union power_supply_propval val;
	if (!smblib_get_prop_dc_present(chg, &val))
		smblib_err(chg, "IRQ: dcin-plugin %s\n",
			!!val.intval ? "attached" : "detached");
}
#endif
...
irqreturn_t override_handle_usb_plugin(int irq, void *data) {
	...
	// Call original here
	smblib_handle_usb_plugin(irq, data);
	...
	if (!vbus) {
		...
		workaround_charging_without_cc_reserve(chg);
...
irqreturn_t override_handle_dc_debug(int irq, void *data) {
	...
	smblib_dbg(chg, PR_INTERRUPT, "IRQ: %s; present:%d, online:%d, icl_settled=%d\n",
		irq_data->name, !smblib_get_prop_dc_present(chg, &val) ? val.intval : -1,
		!smblib_get_prop_dc_online(chg, &val) ? val.intval : -1,
		!smblib_get_charge_param(chg, &chg->param.icl_stat, &icl_ua) ? icl_ua : -1);
