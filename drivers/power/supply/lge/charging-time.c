...
static void chgtime_dworkf(struct work_struct *work) {
	...
	if (!time_me.chgtime_get(&power)
		|| time_me.profile_power != power) {
		pr_chgtime(UPDATE, "One more sampling (power:%dmw)\n",
			power);

		/* Input power 0mW or full raw-soc 0 is not permitted to be updated */
		if (power != 0) {
			/* Update information */
			time_me.profile_power = power;
		}
...
int charging_time_remains(int rawsoc) {
	...
	/* At this time, the returning value is the result of profile_remaining(rawsoc),
