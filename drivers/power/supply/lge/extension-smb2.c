...
static void debug_polling(struct smb_charger* charger) {
	...
	if (presence_dc) { // On DC(Wireless) charging
		...
		pr_info("PMINFO: [WLC] VMAX:%d, VNOW:%d, IMAX:%d,"
			" IWLC:%d(now)<=%d(set)<=%d(cap), AICL:0x%02x, CMD:0x%02x\n",
			wlc_vmax, wlc_vnow, wlc_imax,
			wlc_inow, wlc_aicl, capping_idc, stat_aicl, stat_cmd);
	}
	if (presence_usb && presence_dc) {
	...
	if (type == POWER_SUPPLY_TYPE_USB_DCP || type == POWER_SUPPLY_TYPE_USB_CDP) {
		...
		if (!qcfaking) {
			...
			int voltage_mv = /* 5V fixed for DCP and CDP */
				5000;
			int current_ma = !smb2_usb_get_prop(usb, POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED, &buf)
				? buf.intval / 1000 : 0;
	...
	case POWER_SUPPLY_PROP_PD_IN_HARD_RESET:
		if (val->intval && chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
			...
			if (veneer) {
				...
				pr_info("Setting floated charger by force\n");
			}
		}
