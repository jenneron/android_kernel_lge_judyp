...
static struct protection_battemp {
	...
	bool (*get_protection_battemp)(int* temperature, bool* charging);
...
const	int period_ms_unknown;
	...
	int warning_ma_charge;
	int warning_mv_float;
	...
	.warning_ma_charge = BATTEMP_NOTREADY,
	.warning_mv_float = BATTEMP_NOTREADY,
};
...
static int health_to_icharge(int health) {
	...
	case POWER_SUPPLY_HEALTH_WARM :
	case POWER_SUPPLY_HEALTH_COOL :
		return battemp_me.warning_ma_charge;
	case POWER_SUPPLY_HEALTH_GOOD :
	case POWER_SUPPLY_HEALTH_UNKNOWN :
...
static int health_to_vfloat(int health) {
	...
	case POWER_SUPPLY_HEALTH_HOT :
	case POWER_SUPPLY_HEALTH_WARM :
		return battemp_me.warning_mv_float;
	case POWER_SUPPLY_HEALTH_GOOD :
	case POWER_SUPPLY_HEALTH_COOL :
	case POWER_SUPPLY_HEALTH_COLD :
	case POWER_SUPPLY_HEALTH_UNKNOWN :
...
static void polling_work(struct work_struct* work) {
	...
	if (battemp_me.get_protection_battemp(&temperature_now, &charging_now)) {
		health_new = polling_health(battemp_me.battemp_health,
			temperature_now);
	}
	else {
		pr_battemp(UPDATE, "temperature is not valid.\n");
		health_new = battemp_me.battemp_health; // To bypass this work.
	}

	if (battemp_me.battemp_charging != charging_now
		|| battemp_me.battemp_health != health_new) {

		// limit current/vfloat here
		int updated_icharge = charging_now ?
			health_to_icharge(health_new) : VOTE_TOTALLY_RELEASED;
		int updated_vfloat = charging_now ?
			health_to_vfloat(health_new) : VOTE_TOTALLY_RELEASED;

		veneer_voter_set(&battemp_me.voter_icharge, updated_icharge);
		veneer_voter_set(&battemp_me.voter_vfloat, updated_vfloat);

		// configure wakelock
		if ((health_new != POWER_SUPPLY_HEALTH_GOOD && charging_now)
			|| (health_new == POWER_SUPPLY_HEALTH_HOT)) {
		...
		// logging for changes
		pr_battemp(UPDATE, "%s(%d) -> %s(%d), temperature=%d, charging=%d\n",
			health_to_string(battemp_me.battemp_health),
				health_to_index(battemp_me.battemp_health),
			health_to_string(health_new),
				health_to_index(health_new),
			temperature_now, charging_now);

		// update member status in 'battemp_me'
		battemp_me.battemp_health = health_new;
		battemp_me.battemp_charging = charging_now;
	...
	schedule_delayed_work(to_delayed_work(work), health_to_period(health_new));
	return;
}

static bool battemp_create_parsedt(struct device_node* dnode, int mincap) {
	int rc = 0, warning_ma_pct = 0;
	...
	OF_PROP_READ_S32(dnode, battemp_me.warning_mv_float,
			"warning-mv-float", rc);
	OF_PROP_READ_S32(dnode, warning_ma_pct, "warning-ma-pct", rc);
	battemp_me.warning_ma_charge = mincap * warning_ma_pct / 100;
...
static bool battemp_create_preset(bool (*feed_protection_battemp)(int* temperature, bool* charging),
...
bool protection_battemp_create(struct device_node* dnode, int mincap,
	bool (*feed_protection_battemp)(int* temperature, bool* charging),
	void (*back_protection_battemp)(int health, int micharge, int mvfloat)) {
...
void protection_battemp_destroy(void) {
	...
	battemp_me.warning_ma_charge = BATTEMP_NOTREADY;
	battemp_me.warning_mv_float = BATTEMP_NOTREADY;
}
