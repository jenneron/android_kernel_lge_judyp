...
static void lge_bc_dim_set_sw43402(struct dsi_panel *panel, u8 bc_dim_en, u8 bc_dim_f_cnt)
{
	...
	if (bc_dim_en == BC_DIM_ON) {
		((u8 *)(panel->lge.lge_cmd_sets[LGE_DDIC_DSI_BC_DEFAULT_DIMMING].cmds->msg.tx_buf))[22] = bc_dim_f_cnt;
	...
	pr_info("BC_DIM_EN: 0x%x, FRAMES: 0x%x\n",
			...
			((u8 *)(panel->lge.lge_cmd_sets[LGE_DDIC_DSI_BC_DEFAULT_DIMMING].cmds->msg.tx_buf))[22]);
...
static void lge_get_brightness_dim_sw43402(struct dsi_panel *panel)
{
	...
	bc_dim_f_cnt = ((u8 *)(panel->lge.lge_cmd_sets[LGE_DDIC_DSI_BC_DEFAULT_DIMMING].cmds->msg.tx_buf))[22];
