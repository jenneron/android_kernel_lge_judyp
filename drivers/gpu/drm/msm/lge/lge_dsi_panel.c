...
static int set_lp(struct dsi_panel *panel, enum lge_panel_lp_state lp_state, enum dsi_cmd_set_type cmd_set_type)
{
	...
	} else if (cmd_set_exists(panel, cmd_set_type)) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_err("display_info: %s - %s %d x %d %s\n", lge_man_name, lge_ddic_name, panel->cur_mode->timing.h_active, panel->cur_mode->timing.v_active, panel->panel_mode?"CMD":"VIDEO");

	return sprintf(buf, "display_info: %s - %s %d x %d %s\n",
						lge_man_name,
						lge_ddic_name,
						panel->cur_mode->timing.h_active,
						panel->cur_mode->timing.v_active,
						panel->panel_mode?"cmd":"video");
}
...
int lge_mdss_dsi_panel_cmd_read(struct dsi_panel *panel, u8 cmd, int cnt, char* ret_buf)
{
	...
	ops = panel->host->ops;
	ret = ops->transfer(panel->host, &msg);
				...
				enum dsi_cmd_set_type type, u8 reg, int nth_cmds)
{
	...
	if (count == 0) {
		...
		if (update_cmd) {
			...
			payload++;
			lge_mdss_dsi_panel_cmd_read(panel, reg, update_count, payload);
		} else {
