...
#define JUDY_1226_JUDY_NEO_FIRMWARE_VER_0_CHECKSUM	0x00014526
		...
		uint16_t e2p_data_length, uint16_t reg_str_addr)
{
	...
	camera_io_dev_read_seq(&(local_cam_ois_t->io_master_info), e2p_str_addr,
		e2p_cal_data, CAMERA_SENSOR_I2C_TYPE_WORD, END_FIRST_BLOCK_CAL_DATA);
	camera_io_dev_read_seq(&(local_cam_ois_t->io_master_info), e2p_str_addr + STR_SECOND_BLOCK_CAL_DATA,
		e2p_cal_data + END_FIRST_BLOCK_CAL_DATA, CAMERA_SENSOR_I2C_TYPE_WORD, (e2p_data_length - END_FIRST_BLOCK_CAL_DATA));
